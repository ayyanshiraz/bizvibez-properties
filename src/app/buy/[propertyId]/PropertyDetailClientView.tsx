...
const PropertyDetailClientView = ({ propertyData }: { propertyData: ForSaleProperty }) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            setStatusMessage("Error: Could not send your request. Please try again later.");
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="bg-white font-sans">
            ...
            {images.length > 0 && (
                <div className="relative w-full h-[60vh] bg-gray-900">
                    ...
                    <button onClick={prevImage} className="absolute left-4 top-1/2 -translate-y-1/2 bg-black bg-opacity-50 p-2 rounded-full hover:bg-opacity-75"><ChevronLeftIcon /></button>
                    <button onClick={nextImage} className="absolute right-4 top-1/2 -translate-y-1/2 bg-black bg-opacity-50 p-2 rounded-full hover:bg-opacity-75"><ChevronRightIcon /></button>
                </div>
            )}
                    ...
                    <div className="lg:col-span-2">
                        ...
                        <div>
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                {images.map((img, i) => (
                                    <div key={i} className="h-48 rounded-md overflow-hidden">
                    ...
                    <div className="lg:col-span-1">
                        <div className="sticky top-10 border border-gray-300 rounded-sm p-6 shadow-lg">
                            ...
                            >
                                ...
                                <input type="text" name="fullName" placeholder="Full Name" required value={formData.fullName} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-sm focus:outline-none focus:ring-1 focus:ring-purple-500 text-black placeholder-gray-500" />
                                
                                <input type="email" name="email" placeholder="Email" required value={formData.email} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-sm focus:outline-none focus:ring-1 focus:ring-purple-500 text-black placeholder-gray-500" />
                                
                                <div className="flex items-stretch w-full border border-gray-300 rounded-sm focus-within:ring-1 focus-within:ring-purple-500">
                                ...
                                <textarea name="message" rows={5} placeholder="I'm interested in this property..." required value={formData.message} onChange={handleChange} className="w-full px-4 py-2 border border-gray-300 rounded-sm focus:outline-none focus:ring-1 focus:ring-purple-500 text-black placeholder-gray-500"></textarea>
                                
                                {statusMessage && (
                                    <div className={`p-3 text-center rounded-sm text-sm ${statusMessage.toLowerCase().startsWith('error') ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>
                                        {statusMessage}
                                    </div>
                                )}
                                
                                <button type="submit" style={{ backgroundColor: '#891e6d' }} disabled={loading} className="w-full text-white font-bold py-3 px-4 rounded-sm hover:opacity-90 transition-opacity disabled:opacity-70 disabled:cursor-not-allowed">
