    ...
    return (
        <div className="overflow-hidden group rounded-lg shadow-md transition-all duration-300 hover:shadow-2xl hover:scale-105" onMouseEnter={startCarousel} onMouseLeave={stopCarousel}>
            <div className="relative overflow-hidden">
                ...
                <div className={`absolute bottom-4 left-4 text-lg font-bold px-4 py-2 rounded-md transition-colors duration-300 ${isHovered ? 'bg-[#970060]' : 'bg-black bg-opacity-70'} text-white`}>{plot.price}</div>
            ...
            <div className={`py-6 px-4 transition-colors duration-300 ${isHovered ? 'bg-[#970060]' : 'bg-white'}`}>
        ...
        <Link href={`/off-plan/${property.id}`} className="block group">
            <div className="bg-white rounded-xl shadow-md overflow-hidden h-full flex flex-col transition-all duration-300 hover:shadow-2xl hover:scale-105">
                ...
                <div className="p-6 border-b border-gray-200 flex-grow">
                     ...
                     <p className="text-sm text-gray-800">Start From <span className="text-lg font-bold text-[#970060]">AED {formattedPrice}</span></p>
                </div>
                <div className="px-6 py-4 bg-gray-50 transition-colors duration-300 group-hover:bg-[#970060]">
    ...
    return (
        <section ref={sectionRef} className={`py-20 bg-gray-50 transition-all duration-1000 ease-out transform ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-1/4'}`}>
            <div className="container mx-auto px-6">
                <div className="text-center mb-12"><h2 className="text-4xl font-bold text-[#970060]">Featured Properties in Dubai</h2><p className="text-gray-600 mt-4 max-w-2xl mx-auto">Explore a curated selection of premier off-plan projects from the city's top developers.</p></div>
                <div className="relative">
                ...
                {totalSlides > 1 && (<div className="flex justify-center space-x-2 mt-8">{slides.map((_, index) => (<button key={index} onClick={() => setCurrentSlide(index)} className={`w-3 h-3 rounded-full transition-all duration-300 ${currentSlide === index ? 'bg-[#970060] scale-125' : 'bg-gray-300'}`}></button>))}</div>)}
                <div className="text-center mt-12"><Link href="/off-plan" className="inline-block bg-[#970060] text-white font-bold py-3 px-8 rounded-lg transition-transform duration-300 hover:scale-105 shadow-lg hover:shadow-xl">View All Properties</Link></div>
            </div>
        </section>
    );
};
    ...
    return (
        <>
                ...
                <div className="relative z-20 flex flex-col items-center px-4 w-full mb-20">
                    ...
                    <div className="w-full max-w-4xl">
                        ...
                        <div className="relative" onKeyDown={handleKeyDown}>
                            <div className="bg-white p-4 rounded-lg shadow-lg flex flex-col md:flex-row items-center gap-4"><select className="w-full md:w-auto p-3 bg-gray-100 rounded-md text-gray-600 focus:outline-none border-r-8 border-transparent"><option>Property Type</option><option>Apartment</option><option>Villa</option><option>Townhouse</option></select><input type="text" placeholder="Search by Location, Title, or Property ID..." className="w-full p-3 text-gray-700 focus:outline-none border-t border-b md:border-none" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onFocus={() => setIsSearchFocused(true)} onBlur={() => setTimeout(() => setIsSearchFocused(false), 200)} /><button style={{ backgroundColor: '#970060' }} className="w-full md:w-auto text-white font-bold py-3 px-8 rounded-md hover:opacity-90 transition duration-300" onClick={() => handleSearchAction()}>Search</button></div>
            ...
            <section className="py-20 bg-cover bg-center text-white" style={{ backgroundImage: "linear-gradient(rgba(0,0,0,0.75), rgba(0,0,0,0.75)), url('/guides/hero/downtown.jpeg')" }}>
                <div className="container mx-auto px-6">
                    <div className="text-center mb-16"><h2 className="text-4xl md:text-5xl font-bold">Dubai Real Estate Market Report</h2><p className="text-xl text-gray-300 mt-2">SEPTEMBER 2025</p></div>
                    <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16 items-center">
                        <div className="space-y-12 text-center md:text-left"><div><h3 className="text-6xl font-bold">54.3B</h3><p className="text-gray-300">TOTAL SALES VALUE IN AED</p><p className="text-green-400 mt-1 text-lg">▲ 21.2% <span className="text-gray-400">increase when compared to September 2024</span></p></div><div><h3 className="text-6xl font-bold">20,127</h3><p className="text-gray-300">TOTAL SALES TRANSACTIONS</p><p className="text-green-400 mt-1 text-lg">▲ 11.3% <span className="text-gray-400">increase when compared to September 2024</span></p></div></div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8"><div className="flex flex-col items-center"><InteractivePieChart offplanPercent={66} secondaryPercent={34} color1="#D3D3D3" color2="#970060" /><h4 className="font-bold text-xl mt-4">VALUE</h4><ul className="text-sm mt-2 space-y-1 text-left"><li className="flex items-center"><span className="w-3 h-3 rounded-full bg-[#D3D3D3] mr-2"></span>OFFPLAN: 46.6B</li><li className="flex items-center"><span className="w-3 h-3 rounded-full bg-[#970060] mr-2"></span>SECONDARY: 17.6B</li></ul></div><div className="flex flex-col items-center"><InteractivePieChart offplanPercent={72} secondaryPercent={28} color1="#D3D3D3" color2="#970060" /><h4 className="font-bold text-xl mt-4">VOLUME</h4><ul className="text-sm mt-2 space-y-1 text-left"><li className="flex items-center"><span className="w-3 h-3 rounded-full bg-[#D3D3D3] mr-2"></span>OFFPLAN: 14,932</li><li className="flex items-center"><span className="w-3 h-3 rounded-full bg-[#970060] mr-2"></span>SECONDARY: 5,195</li></ul></div></div>
                    </div>
                </div>
            </section>
            <section className="py-20 bg-gray-50">
                <div className="container mx-auto px-6">
                    <div className="text-center mb-16"><h2 className="text-4xl font-bold text-[#970060]">Featured Land For Sale</h2><p className="text-gray-600 mt-4 max-w-2xl mx-auto">Discover exclusive investment opportunities with our curated selection of premium land plots in Dubai's most desirable locations.</p></div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">{featuredPlots.map((plot, index) => (<PlotCard key={index} plot={plot} />))}</div>
                    <div className="text-center mt-16"><a href="/land-for-sale" className="inline-block bg-[#970060] text-white font-bold py-3 px-8 rounded-md transition-transform duration-300 hover:scale-105">View All Land Plots</a></div>
                </div>
            </section>
